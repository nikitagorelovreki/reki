...
export class ClientRepository implements ClientRepositoryPort {
  private readonly tableName = 'clients'; // Используем таблицу clients для клиентов
  ...
  async delete(id: string): Promise<void> {
    const result = await this.db.knex(this.tableName).where({ id }).del();
  ...
  getQueryBuilder(): ReturnType<typeof this.db.knex> {
